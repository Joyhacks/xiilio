...
export function AgentCard({
  ...
  return (
    <div
      className={cn(
        "group relative p-4 md:p-6 rounded-xl md:rounded-2xl glass-card transition-all duration-500",
    ...
    >
      ...
      <div className="flex flex-wrap gap-1.5 md:gap-2 mb-4 md:mb-6">
        {features.slice(0, 3).map((feature) => (
          <span
            key={feature}
            className="text-[10px] md:text-xs px-1.5 md:px-2 py-0.5 md:py-1 rounded-md bg-muted/50 text-muted-foreground"
          >
            {feature}
          </span>
        ))}
      ...
      <div className="flex items-center justify-between pt-3 md:pt-4 border-t border-border/50">
        <div>
          <div className={cn("text-xl md:text-2xl font-display font-bold", colors.text)}>
            {tasksCompleted.toLocaleString()}
          </div>
          <div className="text-[10px] md:text-xs text-muted-foreground">Tasks completed</div>
        </div>
        {slug ? (
